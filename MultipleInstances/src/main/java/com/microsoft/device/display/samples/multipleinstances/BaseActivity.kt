...
import kotlinx.android.synthetic.main.layout_first_screen.*
import kotlinx.android.synthetic.main.layout_second_screen.*

/**
 * Base Activity that shows one TextView for single screen mode and two TextView's for dual screen mode.
 */
abstract class BaseActivity : AppCompatActivity(), ScreenInfoListener {
    ...
    private fun addShortcuts() {
        val mainActivityIntent = Intent(Intent.ACTION_MAIN, Uri.EMPTY, this, MainActivity::class.java)
        mainActivityIntent.addFlags(Intent.FLAG_ACTIVITY_MULTIPLE_TASK or Intent.FLAG_ACTIVITY_SINGLE_TOP)

        val secondActivityIntent = Intent(Intent.ACTION_MAIN, Uri.EMPTY, this, SecondActivity::class.java)
        mainActivityIntent.addFlags(Intent.FLAG_ACTIVITY_MULTIPLE_TASK or Intent.FLAG_ACTIVITY_SINGLE_TOP)
